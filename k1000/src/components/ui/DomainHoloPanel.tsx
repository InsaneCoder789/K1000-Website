...
export default function DomainHoloPanel({ domain, onClose }: { domain: Domain; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Close Button - Fixed for Mobile accessibility */}
        <button 
          onClick={onClose} 
        ...
        <div className="w-full md:w-[350px] border-b md:border-b-0 md:border-r border-white/5 bg-black flex flex-col shrink-0">
          <div className="p-6 md:p-8 border-b border-white/5 pr-16 md:pr-8">
             <div className="flex items-center gap-2 mb-3 md:mb-4">
                ...
                <span className={`${orbitron} text-[8px] md:text-[9px] text-cyan-500 tracking-[0.3em] font-bold uppercase`}>Leadership</span>
             </div>
             <h1 className={`${conthrax} text-xl md:text-2xl text-white uppercase leading-tight tracking-tighter`}>
                {titleDecrypted}
             </h1>
          </div>
          ...
          <div className="flex md:flex-col overflow-x-auto md:overflow-y-auto p-4 md:p-6 gap-4 md:space-y-8 custom-scroll scrollbar-hide">
             <div className="min-w-[260px] md:min-w-full">
                <h3 className={`${orbitron} text-[8px] md:text-[9px] text-white/30 tracking-[0.2em] uppercase mb-3 md:mb-4 font-bold`}>Director</h3>
                {director ? <LeaderCard leader={director} /> : <div className="h-40 md:h-44 border border-dashed border-white/10 rounded-lg" />}
             </div>
             <div className="min-w-[260px] md:min-w-full">
                <h3 className={`${orbitron} text-[8px] md:text-[9px] text-white/30 tracking-[0.2em] uppercase mb-3 md:mb-4 font-bold`}>Deputy Director</h3>
          ...
          <div className="max-w-3xl space-y-10 md:space-y-16 pb-12 md:pb-0">
            ...
            <section className="space-y-4 md:space-y-6">
              <div className="flex items-center gap-4">
                ...
                <h2 className={`${conthrax} text-[9px] md:text-[10px] tracking-[0.4em] text-cyan-500 uppercase`}>Domain Overview</h2>
              </div>
            ...
            {domain.focusAreas && (
              <section>
                <h2 className={`${conthrax} text-[9px] md:text-[10px] tracking-[0.4em] text-white/30 uppercase mb-6 md:mb-8`}>Focus Areas</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                  {domain.focusAreas.map((area, i) => (
                    ...
                    >
                      ...
                      <span className="text-[9px] md:text-[11px] text-white/70 uppercase tracking-[0.15em] font-bold">{area}</span>
                    </div>
                  ))}
                </div>
              </section>
            ...
            <section className="p-6 md:p-8 bg-white/[0.01] border border-white/5 rounded-2xl relative">
               <h2 className={`${orbitron} text-[8px] md:text-[9px] text-white/20 tracking-[0.3em] uppercase mb-4 md:mb-6 font-bold`}>Operational Description</h2>
            ...
            {domain.applyLink && (
              <div className="pt-4 md:pt-6 flex justify-center md:justify-start">
                ...
                >
                  <span className={`${conthrax} text-[10px] md:text-xs tracking-[0.2em] md:tracking-[0.4em] font-bold uppercase`}>Apply for Membership</span>
  ...
  return (
    <div className="group relative w-full h-40 md:h-52 overflow-hidden border border-white/10 rounded-2xl bg-black transition-all duration-500 hover:border-cyan-500/40">
      ...
      <div className="absolute bottom-0 left-0 w-full p-4 md:p-5">
        <h4 className={`${conthrax} text-white text-[10px] md:text-xs tracking-wider uppercase mb-1`}>{leader.name}</h4>
        <p className={`${orbitron} text-[7px] md:text-[8px] text-white/40 uppercase tracking-widest font-bold`}>{leader.position}</p>
      </div>
    </div>
  );
}